...
try:
    from . import Distance, tile_number
except ImportError:
    from __init__ import Distance, tile_number
...
class weighted_storage_cube:
        ...
        if cube.is_valid():
            if self.cube["count"]:
                for k in self.order:
                    ...
                    self.cube[k] = (
                        min(i[0], o[0]),
                        (self.cube["count"] * i[1] + cube.cube["count"] * o[1])
    ...
    def get_average(self):
        return {
            ...
            "accuracy": max(
                ...
                Distance(
                    (self.cube["longitude"][0], self.cube["latitude"][0]),
                    (self.cube["longitude"][2], self.cube["latitude"][2]),
                )
                / 2,
            ),
    ...
    def get_bbox(self):
        return [
            self.cube["longitude"][0],
            self.cube["latitude"][0],
            self.cube["longitude"][2],
            self.cube["latitude"][2],
        ]
    ...
    def is_valid(self, timestamp=None):
        ...
        return (
            self.cube["count"]
            and (self.cube["time"][2] + self.expire) > timestamp
            and (self.cube["time"][0] - self.expire) < timestamp
        )
    ...
    def dumps(self):
        ...
        for k in self.order:
            dump += struct.pack(
                "=dff",
                self.cube[k][0],
                self.cube[k][1] - self.cube[k][0],
                self.cube[k][2] - self.cube[k][0],
            )
        return dump

    def loads(self, string, offset=0):
        self.cube["count"] = struct.unpack_from("=L", string, offset)[0]
        offset += 4
        for k in self.order:
...
class shelf:
    ...
    def get_average(self):
        ...
        for cube in self.shelf.values():
            cube_avg = cube.get_average()
            cube_box = cube.get_bbox()
            bbox = [
                min(bbox[0], cube_box[0]),
                min(bbox[1], cube_box[1]),
                max(bbox[2], cube_box[2]),
                max(bbox[3], cube_box[3]),
            ]
            for item in self.order:
                avg[item] = (
                    (avg.get(item, 0) * avg.get("count", 0)) + (cube_avg[item] * cube_avg["count"])
                ) / (avg.get("count", 0) + cube_avg["count"])
            avg["count"] = avg.get("count", 0) + cube_avg["count"]
        avg["accuracy"] = max(
            avg.get("accuracy", 10),
            Distance((bbox[0], bbox[1]), (bbox[2], bbox[3])) / 2,
        )
        return avg

    def is_valid(self, timestamp=None):
        return any(cube.is_valid(timestamp) for cube in self.shelf.values())

    def dumps(self):
        return b"".join([cube.dumps() for cube in self.shelf.values()])
...
if __name__ == "__main__":
    print(
        {
            "altitude": 52.5,
            "longitude": 121.51784396,
            "time": 1381266175,
            "latitude": 25.00435646,
            "speed": 0.0,
            "accuracy": 33.0,
        }
    )
    ...
    cube.add_point(
        {
            "altitude": 62.29999923706055,
            "longitude": 121.44472196,
            "time": 1381266174,
            "latitude": 25.00428507,
            "speed": 0.0,
            "accuracy": 5.0,
        }
    )
