...
def get_location(req = {}):
    yar = {}
    if "wifi" in req:
        yar["wifiAccessPoints"] = []
        for ap in req["wifi"]:
            yar["wifiAccessPoints"].append({"macAddress": ap["mac"].upper(), "signalStrength": ap["ss"]})
    try:
        ret = json.loads(urllib.urlopen("https://location.services.mozilla.com/v1/geolocate?key=test", json.dumps(yar)).read().strip())
        pos = {'position':{'latitude':ret["location"]["lat"], 'longitude':ret["location"]["lng"], 'accuracy':ret["accuracy"]}, 'service': 'mozilla'}
        return pos
    except:
        return False
